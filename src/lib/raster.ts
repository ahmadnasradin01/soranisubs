...
export class SubtitleRaster {
  ...
  static async create(
    ...
    const setup = (await fontSetupFor(ass, fallbackFontUrl)) as unknown as {
      availableFonts: Record<string, string | Uint8Array>;
      fallbackFont: string;
    };

    if (customFont?.bytes) {
      const fontKey = customFont.name.toLowerCase();
      setup.availableFonts[fontKey] = customFont.bytes;
      setup.fallbackFont = fontKey;
    }
